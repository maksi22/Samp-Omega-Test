...
void CRPCCallback::Initialize()
{
	...
	CRPC::Add(eRPC::TOGGLE_GRASS, ToggleGrassPlayer);
...
RPC_CALLBACK CRPCCallback::SetPlayerCheckpointEx(RPC_ARGS)
{
	...
	{
	/*	char *thisCheckpoint = (char*)(CHECKPOINT_ADDR);
		...
		*((float*)(thisCheckpoint + 52)) = 0.0;		*/
		// Crash
		((PlaceCheckpoint_t)CHECKPOINT_ADDR)(1, 5, &pos, size, (col & 0xFF000000) >> 24, (col & 0x00FF0000) >> 16, (col & 0x0000FF00) >> 8, col & 0x000000FF, period, pulse, rot_rate, 0.0f, 0.0f, 0.0f, checkZ);
	}
}
...
RPC_CALLBACK CRPCCallback::ToggleGrassPlayer(RPC_ARGS)
{
	bool toggle;
	if (bsData.Read(toggle))
	{
		CGame::ToggleGrassPlayer(toggle);
	}
}
